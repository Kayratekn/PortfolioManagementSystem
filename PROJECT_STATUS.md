...
Generic Benchmark Comparison API is implemented and validated. Provider-independent Benchmark + BenchmarkPrice storage and generic historical import are also ready. The immediate next backend benchmark step is to finalize the concrete validated dataset handoff/provider-symbol contracts for the supported initial benchmarks and bootstrap/import real historical data through the existing generic importer, starting with BIST100 and then S&P500 and NASDAQ100. After real supported benchmark rows exist, add the small public Benchmark Catalog API so the frontend can list the supported user-selectable alternatives. Do not hard-code benchmark logic, fabricate historical observations, infer currencies or bypass the validated importer.
