...
Historical Performance discovery resolved the external-flow direction before implementation: BUY/SELL remain investment transactions, while user-entered DEPOSIT/WITHDRAWAL represent external portfolio cash flows for performance calculations. `transaction_currency`, the PortfolioCashFlow DEPOSIT/WITHDRAWAL foundation and the internal Derived per-currency cash replay foundation are now implemented prerequisites for historical portfolio valuation with cash. The next focused backend slice is **Historical portfolio valuation including derived cash**. That next slice will combine historical asset market value with derived per-currency cash and convert available cash balances to the portfolio base currency using the existing FX foundation. Assets + cash historical valuation, TWR performance and benchmark comparison are not implemented yet. Benchmark provider/source implementation remains a later focused slice and must still use verified provider contracts rather than guessed mappings.
