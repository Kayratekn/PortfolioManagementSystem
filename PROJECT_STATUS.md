...
- **Backend scope status:** **COMPLETE** - approved backend feature scope is implemented and merged to `main`; only documentation/handoff and external cross-team/operational actions remain.
...
| PortfolioSnapshot generation + historical read v1 | Implemented / validated (public API contract) | Existing migration `20260907_0022` remains the storage foundation with unique `(portfolio_id, snapshot_date)` and Decimal `NUMERIC(20,8)` totals for TRY/USD/EUR/GBP. Authenticated `POST /api/v1/portfolios/{portfolio_id}/snapshots?snapshot_date=YYYY-MM-DD` generates an idempotent persisted snapshot from four independent canonical historical Portfolio Valuation calculations for TRY, USD, EUR and GBP on the same date. The owned portfolio row is locked with PostgreSQL `FOR UPDATE` before existing-row lookup/calculation; same-date generation returns the unchanged persisted row and the database unique constraint remains second-line protection. All four valuations must be `COMPLETE` with non-null totals or generation returns 422 and persists nothing. Decimal arithmetic is preserved and values are quantized only at the persistence boundary to 8dp with `ROUND_HALF_UP`; zero and negative totals remain valid. Authenticated `GET /api/v1/portfolios/{portfolio_id}/snapshots?start_date=YYYY-MM-DD&end_date=YYYY-MM-DD` returns persisted snapshots only, with required inclusive dates, maximum 366 inclusive calendar days and deterministic `snapshot_date ASC, id ASC` ordering. No forward-fill, dynamic GET fallback, scheduler, PATCH, DELETE or revision behavior was added. Transaction/PortfolioCashFlow and canonical dynamic historical valuation remain source of truth; valuation, TWR/performance and benchmark comparison do not read snapshots. Real PostgreSQL concurrency and real Uvicorn/JWT HTTP acceptance passed. |
...
| Official TEFAS risk value | Complete / merged | `profilData["riskDegeri"]` is normalized to `risk_value: int | None`; valid official values remain 1..7, while live TEFAS unavailable sentinels `"-"` and `"0"` normalize to `None`. Other malformed, non-integral, boolean, negative, non-finite or out-of-range values remain fail-closed. Detail snapshots persist only `NULL` or 1..7. |
...
| AI integration | Implemented / validated / live accepted | AI integration foundation, immutable AIAnalysis persistence, Portfolio Analysis, Robustness, AI Analysis History/Read, Report Upload persistence, Report Q&A and standalone AI Sentiment backend orchestration are implemented and validated. Real backend-to-Tamer-AI live acceptance passed on 2026-09-09 for Sentiment, Portfolio Analysis, Robustness and Report Q&A, including PostgreSQL AIAnalysis persistence. AI Chat orchestration remains a separate scope decision. |
...
The official TEFAS 1-7 fund risk value is source-confirmed and extracted from exact-matching `profilData["riskDegeri"]`. Valid official values are persisted as 1..7; live TEFAS unavailable sentinels `"-"` and `"0"` normalize to `None` rather than failing the entire detail read. Unsupported zero-like strings and other malformed, non-integral, boolean, negative, non-finite or out-of-range values remain fail-closed. Do not substitute a derived volatility score when the official source value is unavailable. Raw commission values are persisted without percentage conversion, and valor values are persisted as source integer fields without inferred settlement semantics.
...
- Notes domain is implemented with migration `20260907_0020`, following `20260907_0019`. Notes are authenticated user-owned portfolio notes with required `user_id`, `portfolio_id` and `note_text`; no asset/transaction linkage, title, tags or soft-delete behavior exists. The public API now supports create, list, owner-only text update and owner-only hard delete.
- Notes API provides authenticated `POST /api/v1/notes`, `GET /api/v1/notes`, `PATCH /api/v1/notes/{note_id}` and `DELETE /api/v1/notes/{note_id}`. Create validates portfolio ownership through the existing active-portfolio ownership path. PATCH changes only `note_text`, applies the same trim and 1-2000 character validation as create, and forbids extra fields. PATCH/DELETE use note ownership directly; missing and foreign notes both return HTTP 404 with `Note not found.`. DELETE is a hard delete and returns HTTP 204.
- Existing notes remain persisted, listable, editable and deletable by their owner after their portfolio is soft-deleted, while new notes cannot be created for a deleted portfolio.
...
- Notes edit/delete extension completed on 2026-09-11 with no model, schema or migration change; repository methods flush without committing and service methods own commit/rollback.
- Notes edit/delete verification: focused Notes suite **38 passed**; Notes/Auth/Portfolio broader regression **285 passed**; GPT-5.6 Sol High ownership/security review **PASS** with no blocking findings.
- Full backend regression after Notes edit/delete: **2077 passed** with the same **12 known pre-existing non-blocking Starlette HTTP-422 deprecation warnings**.
- Real PostgreSQL + live HTTP Notes edit/delete acceptance: **PASS**; owner PATCH returned updated trimmed text, foreign-user PATCH/DELETE returned canonical 404 `Note not found.`, owner DELETE returned empty HTTP 204, and all temporary smoke users/data were explicitly cleaned up afterward.
...
Benchmark backend implementation is complete and validated, including provider-independent storage/import, real BIST100/SP500/NASDAQ100 historical data, Benchmark Comparison, Benchmark Catalog and completed-close daily Yahoo Finance synchronization. Precious Metals / Borsa Istanbul integration for GOLD, SILVER and PLATINUM is also complete and validated, including historical/current provider ingestion, AssetPrice persistence, valuation/P&L wiring and BIST daily sync auditing. The remaining benchmark step is operational only: coordinate the external one-shot scheduler with the data-integration environment used for the project's daily TEFAS jobs and verify an actual scheduled run. PortfolioSnapshot generation + historical read v1 is now complete and validated, including canonical multi-currency historical valuation, persisted-only read API, PostgreSQL concurrency acceptance and real HTTP acceptance.
...
- **COMPLETE - precious metals:** Gold, silver and platinum Borsa Istanbul integration is implemented and validated. Canonical provider/source, exact TRY/KG -> TRY/GRAM normalization, historical/current ingestion, generic AssetPrice persistence, valuation/P&L support, daily synchronization and DataSyncRun auditing are complete. Historical provider revision semantics remain intentionally protected by conflict/no-overwrite behavior rather than guessed.
- **COMPLETE - PortfolioSnapshot generation/read v1:** Persisted snapshot generation and historical read are implemented and validated. Generation uses independent canonical TRY/USD/EUR/GBP historical valuation, Decimal 8dp persistence, owned-portfolio row locking and same-date idempotency. GET is persisted-only and does not replace dynamic valuation, TWR/performance or benchmark calculation. No automatic scheduler is part of v1.
- **CROSS-TEAM / DEFERRED - X/Twitter ingestion:** `AIAnalysis`, `ReportDocument`, `ReportChunk`, `ExpertSource`, `UserExpertSource`, `SentimentPost` and standalone AI Sentiment backend orchestration are implemented and validated, and live acceptance against Tamer's running AI service is complete. Real X/Twitter post fetching is intentionally deferred until the supervisor/team decides the approved access method/provider (official X API vs another approved method). Backend must not guess or implement a production X connector before that decision. AI Chat orchestration remains a separate final-scope decision.
...
- **FINAL QA COMPLETE:** Full backend verification completed after the live TEFAS risk-sentinel bugfix with **2065 passed** tests and **96.90% `src/` statement coverage**, exceeding the documented **>=70%** requirement. Live provider verification passed for DDE (`"-" -> None`), IGF (`"0" -> None`) and AAJ (`"4" -> 4`); GPT-5.6 Sol High critical review passed with no blocking or material non-blocking findings. The remaining 12 Starlette HTTP-422 deprecation warnings are known, pre-existing and non-blocking.
...
- Live backend-to-AI service HTTP acceptance against Tamer's actual service passed on 2026-09-09 through the real backend HTTP path, including response validation and AIAnalysis persistence.
...
- Live acceptance against Tamer's actual running AI service passed on 2026-09-09 through the authenticated backend endpoint, including real BERT sentiment response validation and AIAnalysis persistence.
...
- Current operational decision (2026-09-09): real X/Twitter post fetching is intentionally deferred until the supervisor/team decides the approved access method/provider (official X API vs another approved method). The existing provider-agnostic persistence, follow/feed and AI sentiment foundations remain complete; no production X connector should be guessed by backend before that decision.
...
## Live AI Service Acceptance

Status: COMPLETE - 2026-09-09

- Tamer's separate AI FastAPI service was started successfully on `127.0.0.1:8001`; backend ran on `127.0.0.1:8000` with the configured `AI_SERVICE_URL=http://127.0.0.1:8001` and 15-second timeout.
- AI health returned `status=online`; backend health returned `status=ok`.
- Direct backend `AiClient` -> real AI service sentiment smoke passed with HTTP 200, correct UTF-8 Turkish text, `bert-base-turkish-sentiment-cased`, and the agreed response contract.
- Authenticated end-to-end standalone Sentiment acceptance passed through Backend -> PostgreSQL eligible-post selection -> Tamer AI -> response validation -> immutable `AIAnalysis` persistence.
- Authenticated Portfolio Analysis live acceptance passed with real backend-derived TEFAS holdings/history and persisted `AIAnalysis`; returned model/formula versions were `mpt_lstm_gemini_v1.2` / `tefas_mpt_rf_0.40`.
- Authenticated Robustness live acceptance passed and persisted `AIAnalysis`; returned model/formula versions were `stress_test_engine_v1.0` / `historical_stress_v1.0`.
- Authenticated Report Q&A live acceptance passed using owned persisted `ReportDocument` / `ReportChunk` context; returned citations were validated against the exact owned chunks and the result was persisted. Returned model version was `gemini-1.5-flash_rag_v1.0` with `formula_version = NULL`.
- All temporary live-acceptance users, portfolios, assets, transactions, TEFAS observations, reports/chunks and AIAnalysis rows were explicitly cleaned up; cleanup verification passed.
- `GEMINI_API_KEY` was intentionally empty during this acceptance and the AI service reported rule-based/fallback simulation mode. Therefore the backend-to-AI HTTP contracts, validation and persistence are live-accepted, while a real Gemini-provider-path smoke remains a separate optional provider verification rather than a backend integration blocker.
- AI `/api/ai/chat` is technically available in the AI service, but backend Chat orchestration remains pending the final scope decision.

## PortfolioSnapshot Generation + Historical Read v1

Status: COMPLETE - 2026-09-09

- Existing `PortfolioSnapshot` storage foundation and migration `20260907_0022` were reused; no migration or schema change was required.
- Added authenticated `POST /api/v1/portfolios/{portfolio_id}/snapshots?snapshot_date=YYYY-MM-DD`.
- Generation acquires the owned Portfolio row with PostgreSQL `FOR UPDATE` before exact-date lookup and calculation.
- Repeating the same `(portfolio_id, snapshot_date)` returns the existing snapshot unchanged and performs no revaluation or overwrite.
- TRY, USD, EUR and GBP totals are calculated independently through the canonical historical `PortfolioValuationService` path using the same `snapshot_date`; one total is never converted into the other snapshot totals.
- All four valuations must be `COMPLETE` with non-null `total_portfolio_value`; otherwise generation returns 422 and persists nothing.
- Financial arithmetic remains Decimal. Snapshot totals are quantized only at the persistence boundary to `Decimal("0.00000001")` using `ROUND_HALF_UP`.
- Zero and negative portfolio totals are intentionally valid.
- Added authenticated `GET /api/v1/portfolios/{portfolio_id}/snapshots?start_date=YYYY-MM-DD&end_date=YYYY-MM-DD`.
- Snapshot history is persisted-only, inclusive, deterministic by `snapshot_date ASC, id ASC`, requires both dates and allows at most 366 inclusive calendar days.
- Ownership isolation preserves canonical 404 `Portfolio not found.` and ownership is checked before semantic range validation.
- PortfolioSnapshot remains derived persisted history only. Transaction/PortfolioCashFlow and canonical dynamic historical valuation remain source of truth; valuation, TWR/performance and benchmark comparison do not read snapshots.
- No scheduler, automatic daily generation, PATCH, DELETE, revision, forward-fill or dynamic GET fallback was added.
- GPT-5.6 Sol High critical correctness review and targeted re-review passed with no blocker, major, minor or release-blocking test gap.
- Focused Snapshot/Valuation verification: **100 passed**.
- Wider Valuation/Performance/Benchmark/Cash Replay/FX/Precious Metals/Snapshot regression: **442 passed**.
- Real PostgreSQL concurrency acceptance passed with two independent sessions: the second generation waited on the real `FOR UPDATE` lock, both calls returned the same snapshot row, exactly one row persisted and no `IntegrityError` escaped.
- Real PostgreSQL + actual Uvicorn/JWT HTTP acceptance passed for authenticated generation, exact Decimal response serialization, same-date idempotency, persisted history, POST/GET ownership isolation and cleanup.
- Current full backend suite: **2036 passed** with **12 known pre-existing non-blocking Starlette `HTTP_422_UNPROCESSABLE_ENTITY` deprecation warnings**.
- `git diff --check` passed; Alembic head remains `20260909_0026`.
