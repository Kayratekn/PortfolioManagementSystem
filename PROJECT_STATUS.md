...
The next planned focused backend slice is **Portfolio Historical Performance + Benchmark Comparison discovery and contract definition**. First confirm the existing historical valuation/price capabilities, benchmark requirements, date-alignment and normalization rules, and the verified benchmark data-source boundary before deciding whether new persistence/models are required.
