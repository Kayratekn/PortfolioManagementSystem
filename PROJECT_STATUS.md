...
Generic Benchmark Comparison API, provider-independent Benchmark storage/import, BIST100 bootstrap/import tooling, and the real BIST100 historical dataset are implemented and validated end-to-end. The current PostgreSQL environment contains 1,251 persisted BIST100 observations from 2021-09-06 through 2026-09-04. The immediate next benchmark step is to obtain and verify the exact S&P500 and NASDAQ100 provider-symbol/dataset contracts and add them through the same metadata/config + generic import path; do not create benchmark-specific comparison logic. Once the initial supported benchmark rows/data are available, add the small public Benchmark Catalog API for frontend selection. Benchmark daily scheduled synchronization/finality remains a later focused slice and must not weaken the current historical-only importer or accept an unfinished same-day close.
