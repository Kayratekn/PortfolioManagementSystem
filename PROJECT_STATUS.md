...
| BIST100 operational bootstrap + generic CSV import tooling | Implemented / validated (operational tooling; production dataset not yet loaded) | Supported benchmark configuration now defines BIST100 with exact metadata `code=BIST100`, `name=BIST 100`, `benchmark_type=MARKET_INDEX`, `native_currency=TRY`, `index_owner=BORSA_ISTANBUL`, `return_type=PRICE_RETURN`, `provider=YAHOO_FINANCE`, `provider_symbol=XU100.IS`, `is_active=true`. A reusable Benchmark bootstrap service creates the row when absent, returns idempotent `ALREADY_EXISTS` for an exact match, and rejects metadata or provider/symbol ownership conflicts without silent mutation. `scripts/bootstrap_benchmarks.py` exposes operational bootstrap. `scripts/import_benchmark_prices.py` provides benchmark-agnostic CSV historical import using canonical `date,close` input, preserving close values as strings through the CSV boundary and delegating Decimal, NUMERIC(20,8), duplicate, revision, active-benchmark, historical-only and atomic transaction rules to the existing parser/import service. No migration, production BIST100 historical dataset, network fetch, Yahoo/yfinance package, scheduler or same-day finality change was added. |
...
Generic Benchmark Comparison API, provider-independent Benchmark storage/import, and BIST100 operational bootstrap + generic CSV import tooling are implemented and validated. No production BIST100 row or historical dataset has yet been persisted. After this feature is merged, the immediate next benchmark step is to receive/verify the prepared historical BIST100 `date,close` dataset, run the supported BIST100 bootstrap on `main`, and import the real historical series through the validated generic CLI. Then add S&P500 and NASDAQ100 through the same metadata/config + generic import path once their exact provider-symbol and dataset contracts are verified. After real supported benchmark rows exist, add the small public Benchmark Catalog API for frontend selection. Do not hard-code comparison logic per benchmark, fabricate observations, infer currencies or bypass the validated importer.
