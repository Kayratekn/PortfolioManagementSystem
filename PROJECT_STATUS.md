...
| Provider-independent Benchmark + BenchmarkPrice storage foundation | Implemented / validated (internal storage only) | Migration `20260903_0016`, following `20260902_0015`, creates provider-independent `benchmarks` and `benchmark_prices` tables. `Benchmark` contains `id`, required unique `code`, required `name`, required `benchmark_type` constrained to `MARKET_INDEX` or `TEFAS_FUND`, required `native_currency` constrained to exactly 3 alphabetic uppercase ASCII letters, required `provider`, required `provider_symbol`, `is_active`, timestamps and unique `(provider, provider_symbol)`. `BenchmarkPrice` contains `id`, required `benchmark_id` FK, required `price_date`, required Decimal `close_value` / `NUMERIC(20,8)` constrained greater than zero, required `source` and timestamps. `BenchmarkPrice` intentionally has no separate currency field; currency authority lives on `Benchmark.native_currency`. Prices are unique by `(benchmark_id, price_date)` and indexed by `(benchmark_id, price_date)`. `BenchmarkRepository` supports `add`, `get_by_id`, `get_by_code` and deterministic active listing. `BenchmarkPriceRepository` supports `add`, exact-date lookup, range lookup, and latest-on-or-before lookup; historical lookup cannot return future data and queries isolate by `benchmark_id`. Currency semantics do not hardcode `TRY`/`USD`/`EUR`/`GBP`, do not guess currencies and do not infer from Asset/TEFAS data. No public benchmark catalog API, BIST100 seed/fetch, provider client/sync, TEFAS benchmark support, benchmark comparison service/API, normalization, FX conversion, TWR changes or benchmark-series fabrication was added. |
...
Provider-independent Benchmark + BenchmarkPrice storage foundation is implemented and validated. Benchmark storage is now ready, and the first target benchmark remains BIST 100. No BIST100 data/provider mapping has been persisted yet; provider/source and the permitted import method must be verified before implementing sync. Benchmark comparison service/API is still not implemented. The immediate next backend step is **BIST 100 provider/data import contract and sync foundation**. Benchmark implementation must use verified provider/data contracts and must not guess mappings or silently fabricate historical series.
