...
1. **Define Realized P/L v1 methodology before implementation**
   - Perform discovery from the existing Transaction and Cost Basis contracts.
   - Lock the exact native-currency Realized P/L v1 financial semantics, historical/as-of behavior, availability rules and service/API boundaries before coding.
   - Do not implement Realized P/L yet.
   - Do not add FX/base-currency Realized P/L, fees/taxes, migrations or schema changes unless separately specified.
