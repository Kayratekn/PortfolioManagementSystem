...
| TEFAS detail snapshot | Implemented | Includes fund category, 1-year category rank, category fund count and raw market-share value |
...
| Official TEFAS risk value | Source confirmed; implementation pending | TEFAS detail page exposes a current 1–7 risk value, but backend extraction/normalization is not implemented yet |
...
- TEFAS detail-page extraction for `fund_category`, `category_rank`, `category_fund_count` and `market_share_raw` is covered by automated tests.
- Short-term evolution metrics were implemented and merged in PR #29.
- The latest known full backend test-suite result before the documentation-only PR #30 was **520 passed**.
- PR #30 changed documentation only; no application code changed after that 520-test run.
- `git diff --check` and staged diff checks passed for the PR #30 documentation update.
...
1. **TEFAS official risk-value discovery and extraction**
   - Identify the exact raw field/source representation on the TEFAS detail-analysis page.
   - Do not guess the provider field name.
   - Normalize the official value as `int | None`.
   - Preserve missing values as `None`.
   - Never replace a missing official value with a derived volatility score.

2. **Implement risk value in the existing detail-snapshot flow**
   - Extend the relevant TEFAS service result.
   - Extend persistence only if required by the agreed snapshot contract.
   - Add/update Alembic migration if the database schema changes.
   - Add focused service/model/repository tests.
   - Run relevant tests first, then the full suite before PR.

3. **Update documentation and `PROJECT_STATUS.md` after implementation**
   - Promote the risk-value capability from source-confirmed/pending to implemented only after tests pass.
   - Preserve unrelated documentation.

4. **Only after the risk-value slice is merged**
   - Re-evaluate the next unresolved TEFAS/data-contract item or return to the next planned backend domain.
   - Do not jump directly into Transaction development without an explicit decision.

## Current open decisions / remaining data gaps

These are not blockers for the current risk-value task:
