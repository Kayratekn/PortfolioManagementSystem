...
Generic Benchmark Comparison API, provider-independent Benchmark storage/import, the initial real historical datasets for BIST100, SP500 and NASDAQ100, and the public Benchmark Catalog API are now implemented and validated end-to-end. The current PostgreSQL environment contains 1,251 persisted BIST100 observations and 1,255 persisted observations each for SP500 and NASDAQ100. Real comparison smoke tests have passed for native-currency and USD-to-TRY historical FX paths, and the frontend can now obtain the active benchmark choices from `GET /api/v1/benchmarks` instead of hard-coding them. The immediate next benchmark slice is daily scheduled synchronization/finality: first define the provider refresh and completed-close finality contract, then add a controlled scheduled update path without weakening the existing historical-only validation or accepting an unfinished same-day close.
