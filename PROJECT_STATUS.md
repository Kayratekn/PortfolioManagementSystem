...
1. **Implement Cost Basis v1 using the documented Moving Weighted Average Cost methodology**
   - Transactions remain the sole source of truth.
   - Replay transactions deterministically by `transaction_date ASC`, then `id ASC`.
   - Use Decimal-only internal calculations with no internal rounding or quantization.
   - Cost basis must be calculated in the asset's native `Asset.currency`; do not infer missing asset currency.
   - Cost-basis implementation is still not complete, and unrealized P/L and realized P/L remain later slices.
