...
Portfolio Historical Performance / TWR foundation is implemented and validated. TWR is now available through the public authenticated performance endpoint, but benchmark comparison is not implemented yet. The immediate next backend step is **Benchmark Comparison discovery and provider contract definition**. Benchmark implementation must use verified provider/data contracts and must not guess mappings or silently fabricate historical series.
