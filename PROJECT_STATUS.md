...
| Cost Basis v1 | Implemented / validated (service-level foundation) | Moving Weighted Average Cost is implemented in `CostBasisService` from transactions as the sole source of truth. Cost basis is derived by deterministic replay ordered by `transaction_date ASC`, then `id ASC`; historical/as-of calculation includes only transactions with `transaction_date <= as_of_date`, so future transactions do not affect historical cost basis. BUY updates the moving weighted average. Partial SELL removes cost using the current average cost and leaves the remaining average unchanged. Full SELL resets cost-basis state; fully sold assets are omitted from positive as-of holdings/results, and a later BUY after full exit starts a fresh cost basis. Calculations use Decimal only with no explicit internal rounding or quantization. Cost basis is in `Asset.currency`; missing or blank currency produces an unavailable item and makes the `CostBasisResult` `INCOMPLETE`, and currency is never inferred. Multiple asset currencies are not summed into a portfolio-level cost-basis total. The implementation is transaction-based and is not restricted to TEFAS/FUND assets. No mutable cost-basis/holdings table, migration, API/controller/response/dependency/main wiring, unrealized P/L, realized P/L or FX conversion of cost basis/P&L was added. |
...
1. **Define the next integration contract before exposing cost basis or implementing P/L**
   - Cost Basis v1 methodology and service-level implementation are complete and validated.
   - Do not expose cost basis through a public API until the request/response/dependency/controller contract is explicitly defined.
   - Do not implement unrealized P/L, realized P/L or FX conversion of cost basis/P&L until their contracts are explicitly defined.
