...
Generic Benchmark Comparison API, provider-independent Benchmark storage/import, and the initial real historical datasets for BIST100, SP500 and NASDAQ100 are now implemented and validated end-to-end. The current PostgreSQL environment contains 1,251 persisted BIST100 observations and 1,255 persisted observations each for SP500 and NASDAQ100. Real comparison smoke tests have passed for native-currency and USD-to-TRY historical FX paths. The immediate next benchmark feature is the small public Benchmark Catalog API so the frontend can retrieve and present the active supported benchmarks instead of hard-coding them. Benchmark daily scheduled synchronization/finality remains a later focused slice and must preserve the existing historical-only validation and must not accept an unfinished same-day close.
