...
| Realized P/L v1 | Documented methodology / not implemented | Realized P/L v1 methodology is now documented in `AGENTS (2).md` as native-currency-only, cumulative through one required `as_of_date`, transaction-sourced, Decimal-only and based on the existing Moving Weighted Average Cost replay using the SELL-point average cost. Service/API remain pending. The implementation slice must not add FX/base-currency P/L, portfolio totals, fees/taxes, migrations, tables or schema changes. |
...
1. **Implement service-level Realized P/L v1 using the documented methodology**
   - Build the service-level Realized P/L v1 calculation from the documented native-currency, Decimal-only Moving Weighted Average replay semantics.
   - Keep transactions as the sole source of truth and use the SELL-point average cost; do not derive Realized P/L only from ending Cost Basis results.
   - Service/API remain pending before this slice starts; add service-level implementation first.
   - Do not add FX/base-currency P/L, portfolio totals, fees/taxes, period/from-to behavior, migrations, tables or schema changes in this implementation slice.
