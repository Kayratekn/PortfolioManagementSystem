...
Historical Performance discovery resolved the external-flow direction before implementation: BUY/SELL remain investment transactions, while user-entered DEPOSIT/WITHDRAWAL represent external portfolio cash flows for performance calculations. `transaction_currency`, the PortfolioCashFlow DEPOSIT/WITHDRAWAL foundation, the internal Derived per-currency cash replay foundation and Historical portfolio valuation including derived cash are now implemented prerequisites for performance calculation. The next focused backend slice is **Portfolio historical performance / TWR foundation**. That next slice should use historical total portfolio value including cash and explicit DEPOSIT/WITHDRAWAL external flows, while BUY/SELL remain internal trades. TWR performance and benchmark comparison are not implemented yet. Benchmark provider/source implementation remains a later focused slice and must still use verified provider contracts rather than guessed mappings.
