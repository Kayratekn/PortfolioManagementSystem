...
| S&P500 + NASDAQ100 benchmark metadata/contracts | Implemented / validated (real historical datasets prepared; production rows not yet loaded) | The generic supported-benchmark registry now includes `SP500` with `name=S&P 500`, `native_currency=USD`, `index_owner=SP_DOW_JONES_INDICES`, `return_type=PRICE_RETURN`, `provider=YAHOO_FINANCE`, `provider_symbol=^GSPC`; and `NASDAQ100` with `name=NASDAQ-100`, `native_currency=USD`, `index_owner=NASDAQ`, `return_type=PRICE_RETURN`, `provider=YAHOO_FINANCE`, `provider_symbol=^NDX`. Both remain `MARKET_INDEX` and active. Existing generic bootstrap/import/comparison behavior is unchanged. Data-team 5-year daily Close datasets were received for both, canonicalized to the backend `NUMERIC(20,8)` contract and accepted 1255/1255 by the existing backend CSV reader/parser for date range 2021-09-07 through 2026-09-04. Real PostgreSQL rollback bootstrap verified CREATED -> ALREADY_EXISTS for both codes. No migration, benchmark-specific comparison logic, runtime Yahoo/yfinance dependency or production SP500/NASDAQ100 Benchmark/BenchmarkPrice persistence was added yet. |
...
Generic Benchmark Comparison API, provider-independent Benchmark storage/import, the real BIST100 historical dataset, and the locked SP500/NASDAQ100 metadata/config contracts are implemented and validated. The current PostgreSQL environment contains 1,251 persisted BIST100 observations; SP500 and NASDAQ100 production Benchmark rows and historical prices have not yet been persisted. Their validated canonical datasets each contain 1,255 observations covering 2021-09-07 through 2026-09-04. After this feature is merged, the immediate next benchmark step is to bootstrap `SP500` and `NASDAQ100` on `main`, import their real historical series through the existing generic CLI, verify persisted counts/date bounds/provenance, and run real-data Benchmark Comparison API smoke tests including USD-to-portfolio-base historical FX behavior where applicable. After the initial supported benchmark rows/data are operational, add the small public Benchmark Catalog API for frontend selection. Benchmark daily scheduled synchronization/finality remains a later focused slice and must not weaken the historical-only importer or accept an unfinished same-day close.
