...
Provider-independent Benchmark + BenchmarkPrice storage plus Benchmark metadata and generic historical price import foundation are implemented and validated. Benchmark storage/import is now ready, and the first target benchmark remains BIST 100. No BIST100 catalog row/seed, XU100/XU100.IS mapping or data/provider mapping has been persisted yet. The immediate next backend step is to **finalize the approved BIST100 dataset/provider contract with the data team**, then bootstrap/import BIST100 into the existing generic benchmark foundation. Benchmark comparison service/API remains a later slice. Benchmark implementation must use verified provider/data contracts and must not guess mappings or silently fabricate historical series.
