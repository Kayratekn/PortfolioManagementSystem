...
Generic Benchmark Comparison API, provider-independent Benchmark storage/import, the initial real historical datasets for BIST100, SP500 and NASDAQ100, and the public Benchmark Catalog API are now implemented and validated end-to-end. The current PostgreSQL environment contains 1,251 persisted BIST100 observations and 1,255 persisted observations each for SP500 and NASDAQ100. Real comparison smoke tests have passed for native-currency and USD-to-TRY historical FX paths, and the frontend can now obtain the active benchmark choices from `GET /api/v1/benchmarks` instead of hard-coding them. Benchmark daily scheduled synchronization/finality is now implemented and validated end-to-end. The remaining benchmark operational step is to configure the one-shot sync script in the external host scheduler for a safe post-close run time (approximately 02:00 Europe/Istanbul) and verify an actual scheduled run; no in-process background scheduler or unfinished same-day Close acceptance is planned.
