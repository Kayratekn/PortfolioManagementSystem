...
| Unrealized P/L v1 | Methodology documented / not implemented | Native-currency-only Unrealized P/L v1 semantics are selected and documented. Planned formula is `native_market_value = quantity * selected_price` and `native_unrealized_pl = native_market_value - total_cost_basis`, using Decimal only with no internal rounding or quantization; negative, zero and positive values are valid. Cost basis must come from the existing Moving Weighted Average Cost result for transactions with `transaction_date <= as_of_date`, and market value must use existing latest-on-or-before valuation price-selection semantics for the same `as_of_date` (`YAT`/`EMK`/`GYF`/`GSYF` TEFAS NAV, `BYF` `exchange_bulletin_price`, no BYF NAV fallback). V1 must not expose converted cost basis, portfolio-currency unrealized P/L, FX-adjusted unrealized P/L or portfolio-level total Unrealized P/L; different native currencies must never be summed directly. `FX_UNAVAILABLE` alone does not make native Unrealized P/L unavailable when price, `Asset.currency` and native Cost Basis are available. Missing native inputs such as `PRICE_UNAVAILABLE`, `ASSET_CURRENCY_UNAVAILABLE`, `UNSUPPORTED_ASSET` or unavailable Cost Basis make the item unavailable, preserving the concrete reason when possible. Current market-data support remains unchanged, so manual/non-TEFAS assets remain `UNSUPPORTED_ASSET` for Unrealized P/L v1 without a supported valuation price. Planned result shape is `UnrealizedPlResult(portfolio_id, as_of_date, status, items)` with per-item quantity, cost-basis, selected-price, native market value and native Unrealized P/L fields; empty portfolios are `COMPLETE`, fully sold assets are omitted, and asset-set or quantity disagreement between Cost Basis and valuation for the same as-of date is an internal invariant violation. Unrealized P/L percentage/return, portfolio-base-currency P/L, historical transaction-date FX cost basis, portfolio totals, realized P/L, fees/commissions/taxes, API endpoint and migration/table/schema changes remain out of scope. |
...
1. **Implement service-level Unrealized P/L v1 using the documented native-currency contract**
   - Unrealized P/L v1 methodology and currency semantics are selected and documented, but not implemented yet.
   - Use the existing Cost Basis v1 result and valuation price-selection semantics for the same `as_of_date`.
   - Do not implement realized P/L, portfolio-base-currency P/L or FX conversion of cost basis/P&L in this slice.
