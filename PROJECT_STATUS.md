...
Historical Performance discovery resolved the external-flow direction before implementation: BUY/SELL remain investment transactions, while user-entered DEPOSIT/WITHDRAWAL represent external portfolio cash flows for performance calculations. `transaction_currency` and the PortfolioCashFlow DEPOSIT/WITHDRAWAL foundation are now persisted prerequisites for future cash replay. The next focused backend slice is **Derived per-currency cash replay foundation**. That next slice will derive cash per currency from `DEPOSIT +amount`, `WITHDRAWAL -amount`, `BUY -quantity*unit_price` and `SELL +quantity*unit_price`; cash replay is not implemented yet. After that foundation is stable, continue in controlled slices with historical assets + cash valuation, TWR performance, and then benchmark comparison. Benchmark provider/source implementation remains a later focused slice and must still use verified provider contracts rather than guessed mappings.
