...
- **Current backend stage:** Authentication and database foundation implemented; auth flow verified locally with both SQLite and PostgreSQL; portfolio domain not started
...
The current repository only contains the backend foundation and the first verified user authentication flow. Local verification has been completed with both SQLite and PostgreSQL.
...
1. Keep auth stable across both verified local databases: SQLite and PostgreSQL.
